---
title: "COMPASS Synopitic Porewater Nutrients 2022-2024"
...
#Plot standard Curve or Curves 
  #v-Nox
std_curve_NOxa <- ggplot(stds_NOxa, aes(x = Conc, y = Absorbance)) +
  geom_point(size=3) +
  ...
  annotate("text", x = 0.75, y = max(stds_NOxa$Absorbance), 
           label = paste("r^2 =", round(NOx_r2b, 4)),
           color = "black", size = 4)
print(std_curve_NOxa)

std_curve_NOxb <- ggplot(stds_NOxb, aes(x = Conc, y = Absorbance)) +
  geom_point(size=3) +
  ...
  labs(x="Concentration (ppm)",
       y="Absorbance",
       title = "NOx Standard Curve b")+ 
  annotate("text", x = 0.75, y = max(stds_NOxb$Absorbance), 
           label = paste("r^2 =", round(NOx_r2b, 4)),
           color = "black", size = 4)
print(std_curve_NOxb)
...
print(std_curve_PO4)
  ...
  #SO4 
std_curve_SO4a <- ggplot(stds_SO4a, aes(x = Conc, y = Absorbance)) +
  geom_point(size=3) +
  ...
  annotate("text", x = 10, y = max(stds_SO4a$Absorbance), label = paste("r^2 =", round(SO4_r2a, 4)),
          color = "black", size = 4)
print(std_curve_SO4a)

std_curve_SO4b <- ggplot(stds_SO4b, aes(x = Conc, y = Absorbance)) +
  geom_point(size=3) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 3, raw = TRUE), se = FALSE, color = "grey50") +
  facet_wrap(~Run_Date) +
  theme_bw() + 
  labs(x="Concentration (ppm)",
       y="Absorbance",
       title = "SO4 Standard Curve")+ 
  annotate("text", x = 10, y = max(stds_SO4b$Absorbance), label = paste("r^2 =", round(SO4_r2b, 4)),
          color = "black", size = 4)
print(std_curve_SO4b)
...
#Pull out test stds 
red_eff_all <- stds %>%
  filter(Test %in% c("Nitrate (NOx)", "Nitrite"))

dates_red <- sort(unique(red_eff_all$Run_Date))

red_eff <- red_eff_all %>% filter(Run_Date == dates_red[1])
#red_eff_2 <- red_eff_all %>% filter(Run_Date == dates_red[2])
...
final_data_labeled <- final_data4 %>% 
  mutate(
    Project = "COMPASS_LE_Synoptic_Sites_Waters",   # new column with same value on every row
    #Region = "LE",
    #Run_notes = run_notes, 
...
final_data <- final_data_labeled %>%
  ...
  select(Project, Vial_Test_ID, Box, box_trail, Vial_number, Year, Month, Day, Site, Zone, Water, Repl, Depth,  
