...
# Get the unique dates for tests that were redone
dates_NOx <- unique(stds_NOx$Run_Date)
dates_SO4 <- unique(stds_SO4$Run_Date)

# extract curve data based on date or tests that were redone
stds_NOxa <- stds_NOx %>% filter(Run_Date == dates_NOx[1])
stds_NOxb <- stds_NOx %>% filter(Run_Date == dates_NOx[2])

stds_SO4a <- stds_SO4 %>% filter(Run_Date == dates_SO4[1])
stds_SO4b <- stds_SO4 %>% filter(Run_Date == dates_SO4[2])


# NOx x and y
x1a <- stds_NOxa$Absorbance
y1a <- stds_NOxa$Conc

x1b <- stds_NOxb$Absorbance
y1b <- stds_NOxb$Conc
...
# SO4 x and y
x4a <- stds_SO4a$Absorbance
y4a <- stds_SO4a$Conc

x4b <- stds_SO4b$Absorbance
y4b <- stds_SO4b$Conc
...
#NOx
lin_reg_NOxa <- lm(y1a ~ x1a)
NOx_coefsa <- coef(lin_reg_NOxa)                        # intercept and slope
NOx_r2a <- summary(lin_reg_NOxa)$r.squared              # R squared
# Store in a clean dataframe
std_curve_NOx_1a <- data.frame(
  Test = "NOx",
  Intercept = NOx_coefsa["(Intercept)"],
  Slope = NOx_coefsa[2],
  R_squared = NOx_r2a
)

lin_reg_NOxb <- lm(y1b ~ x1b)
NOx_coefsb <- coef(lin_reg_NOxb)                        # intercept and slope
NOx_r2b <- summary(lin_reg_NOxb)$r.squared              # R squared
# Store in a clean dataframe
std_curve_NOx_1b <- data.frame(
  Test = "NOx",
  Intercept = NOx_coefsb["(Intercept)"],
  Slope = NOx_coefsb[2],
  R_squared = NOx_r2b
)
...
triple_reg_SO4a <- lm(y1a ~ x1a + I(x1a^2)+ I(x1a^3))
SO4_coefsa <- coef(triple_reg_SO4a)                        # intercept and slope
SO4_r2a <- summary(triple_reg_SO4a)$r.squared              # R squared
# Store in a clean dataframe
std_curve_SO4_1a <- data.frame(
  Test = "SO4",
  Intercept = SO4_coefsa["(Intercept)"],
  Slope = SO4_coefsa[2],
  R_squared = SO4_r2a
) 

# Fit the 3-polynomial model
triple_reg_SO4b <- lm(y1b ~ x1b + I(x1b^2)+ I(x1b^3))
SO4_coefsb <- coef(triple_reg_SO4b)                        # intercept and slope
SO4_r2b <- summary(triple_reg_SO4b)$r.squared              # R squared
# Store in a clean dataframe
std_curve_SO4_1b <- data.frame(
  Test = "SO4",
  Intercept = SO4_coefsb["(Intercept)"],
  Slope = SO4_coefsb[2],
  R_squared = SO4_r2b
) 
...
stds_combo <- dplyr::bind_rows(stds_NOxa, stds_NOxb, stds_NH3, stds_PO4, stds_SO4a, stds_SO4b, stds_NO2)
std_curve_combo <- dplyr::bind_rows(std_curve_NOx_1a, std_curve_NOx_1b, std_curve_NH3_1, std_curve_PO4_1, std_curve_SO4_1a, std_curve_SO4_1b, std_curve_NO2_1)
...
std_curve_NOx <- ggplot(stds_NOx, aes(x = Conc, y = Absorbance)) +
  ...
  labs(x="Concentration (ppm)",
       y="Absorbance",
       title = "NOx Standard Curve a")+ 
  annotate("text", x = 0.75, y = max(stds_NOxa$Absorbance), 
           label = paste("r^2 =", round(NOx_r2b, 8)),
           color = "black", size = 4)
print(std_curve_NOx)
...
std_curve_NH3 <- ggplot(stds_NH3, aes(x = Conc, y = Absorbance)) +
  ...
  annotate("text", x = 1.7, y = max(stds_NH3$Absorbance), 
           label = paste("r^2 =", round(NH3_r2, 8)),
          color = "black", size = 4)
print(std_curve_NH3)
...
std_curve_PO4 <- ggplot(stds_PO4, aes(x = Conc, y = Absorbance)) +
  ...
  annotate("text", x = 0.25, y = max(stds_PO4$Absorbance), label = paste("r^2 =", round(PO4_r2, 8)),
          color = "black", size = 4)
print(std_curve_PO4)
...
std_curve_SO4 <- ggplot(stds_SO4, aes(x = Conc, y = Absorbance)) +
  ...
  annotate("text", x = 10, y = max(stds_SO4a$Absorbance), label = paste("r^2 =", round(SO4_r2a, 8)),
          color = "black", size = 4)
print(std_curve_SO4)
...
std_curve_NO2 <- ggplot(stds_NO2, aes(x = Conc, y = Absorbance)) +
  ...
  annotate("text", x = 10, y = max(stds_NO2$Absorbance), label = paste("r^2 =", round(NO2_r2, 8)),
          color = "black", size = 4)
print(std_curve_NO2)
...
#Write out to the user whether or not the r2 is above the cutoff of 0.98
  ifelse(std_curve_NOx_1a$R_squared <= r2_cutoff, 
         "NOx Curve r2 is below cutoff! - REASSESS", 
         "NOx Curve r2 GOOD - PROCEED")
    ifelse(std_curve_NOx_1b$R_squared <= r2_cutoff, 
         "NOx Curve r2 is below cutoff! - REASSESS", 
         "NOx Curve r2 GOOD - PROCEED")
  ...
  ifelse(std_curve_SO4_1a$R_squared <= r2_cutoff, 
         "SO4 Curve r2 is below cutoff! - REASSESS", 
         "SO4 Curve r2 GOOD - PROCEED")
  ifelse(std_curve_SO4_1b$R_squared <= r2_cutoff, 
         "SO4 Curve r2 is below cutoff! - REASSESS", 
         "SO4 Curve r2 GOOD - PROCEED")
...
add_log <- as.data.frame(dplyr::bind_rows(std_curve_NOx_1a, std_curve_NOx_1b, std_curve_NH3_1, std_curve_PO4_1, std_curve_SO4_1a, std_curve_SO4_1b, std_curve_NO2_1))
...
Slopes_chk <- ggplot(log, aes(run_date, Slope, col=Test)) + 
  ...
  scale_color_manual(values=c("coral", "coral3", "darkgoldenrod", "darkkhaki", "darkgrey", "grey50", "darkgreen"))
Slopes_chk
...
#write out a flag to the sample dataframe if the r2 is above the cutoff of 0.98
    #Use "|" in case of more days of standard curves like in "NOx_r2a <= r2_cutoff | NOx_r2b <= r2_cutoff"

df_all <- df_all %>%
  mutate(
    
    NOx_flag = ifelse(
      NOx_r2a <= r2_cutoff | NOx_r2b <= r2_cutoff,
      "NOx r2 low",
      ""
    ),
    ...
    SO4_flag = ifelse(
      SO4_r2a <= r2_cutoff | SO4_r2b <= r2_cutoff,
      "SO4 r2 low",
      ""
    ),
...
red_eff_NOx <- stds %>% filter(Test == "Nitrate (NOx)", Target > 0)
rec_eff_NO2 <- stds %>% filter(Test == "Nitrite", Target > 0)
...
if (NOx_mean_red_eff_NOx <= 95) {
    df_all$NOx_flag <- ifelse(
    df_all$NOx_flag != "",
    paste0(df_all$NOx_flag, "; NOx reduction efficiency low"),
    "NOx reduction efficiency low")}

if (NO2_mean_rec_eff_NO2 <= 95) {
    df_all$NO2_flag <- ifelse(
    df_all$NO2_flag != "",
    paste0(df_all$NOx_flag, "; NO2 recovery efficiency low"),
    "NO2 recovery efficiency low")}
...
SO4_chks_percent <- (sum(SO4_chks$SO4_diff_flag == "YES")/nrow(SO4_chks))*100
NO2_chks_percent <- (sum(NO2_chks$NO2_diff_flag == "YES")/nrow(NO2_chks))*100
