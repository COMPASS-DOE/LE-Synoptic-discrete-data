...
invisible(lapply(c( "fluxfinder", "readxl",
                    ...
                    "googledrive", "googlesheets4", "tibble",
                    "matrixStats", "gridExtra", "grid", "tidyverse", "knitr", "kableExtra", "arrow"), 
...
read_li7810_zip <- function(zip_file, tz = "US/Eastern") {
    ...
    # gives us a directory in the system "temporary" space, which will
    ...
    dat_list <- lapply(data_files[1:2], function(f) {
        message("     Reading: ", basename(f))
        ffi_read_LI7810(f) %>% as_tibble()
    })
    
    dat <- bind_rows(dat_list) %>%
        # Some machines can be in the wrong time zone; ensure everything is EST
...
debug(read_li7810_zip)
alldat_list <- lapply(zip_files, read_li7810_zip)
alldat <- bind_rows(alldat_list)
message(now(), " | DONE")
