...
if(any(duplicated(metadat_sn$Plot))) {
...
# Use an existing preprocessed file if one exists
PROCESSED_FILE <- "alldat_matched_2023.rds"
if(file.exists(PROCESSED_FILE)) {
    message("Reading existing processed file ", PROCESSED_FILE)
    alldat_matched <- readRDS(PROCESSED_FILE)
} else {
    # ...otherwise, match data per equipment (considering different serial numbers) uhul!
    matched_list <- list()
    for(sn in unique(alldat$SN)) {
        ...
        alldat_sn$metadat_row <- ffi_metadata_match(
            data_timestamps = alldat_sn$TIMESTAMP,
            start_dates     = metadat_sn$Date,
            start_times     = as.character(metadat_sn$Time_start),
            obs_lengths     = metadat_sn$Obs_length)
        # Drop unmatched rows
        n_before <- nrow(alldat_sn)
        dat_i <- alldat_sn %>% filter(!is.na(metadat_row))
        message("  Matched rows: ", nrow(alldat_sn), " / ", n_before)
    ...
    message("Saving ", PROCESSED_FILE)
    saveRDS(alldat_matched, PROCESSED_FILE)
}
