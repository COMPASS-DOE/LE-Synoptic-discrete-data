...
alldat <- alldat%>%filter(!label %in% c("UPDmonTRtion",
                       "W_22DmonTRtion")) %>%
  mutate(
    label = label %>%
      str_remove_all("[sSeE]") %>%
...
#allflux_filtered <- allflux[!(allflux$Plot_ID %in% fluxes_weird$Plot_ID), ]
...
allflux <- allflux[,-c(26)]

#export as a csv to the folder that you want 
write.csv(allflux, file="Processed Data/COMPASS_Synoptic_LE_SmartChamber_Fluxes_Final_2022_CO2.csv") 
#Change file name
