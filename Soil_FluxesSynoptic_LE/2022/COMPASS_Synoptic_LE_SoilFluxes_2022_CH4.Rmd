...
#if they say retake then we want to replace that with an "a" and if it has -A then we also want to remove that and make it "a" 
fluxes <- fluxes %>%
  mutate(Plot = gsub(pattern = "_", replacement = "-", x = Plot)) %>%
  ...
  mutate(Plot = if_else(str_detect(Plot, "-b"), 
                        str_replace(Plot, "-b", "b"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-01"), 
                        str_replace(Plot, "-01", "-1"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-02"), 
                        str_replace(Plot, "-02", "-2"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-03"), 
                        str_replace(Plot, "-03", "-3"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-04"), 
                        str_replace(Plot, "-04", "-4"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-05"), 
                        str_replace(Plot, "-05", "-5"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-06"), 
                        str_replace(Plot, "-06", "-6"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-07"), 
                        str_replace(Plot, "-07", "-7"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-08"), 
                        str_replace(Plot, "-08", "-8"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-A"), 
                        str_replace(Plot, "-A", "a"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP1"), 
                        str_replace(Plot, "UP1", "UP_1"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP2"), 
                        str_replace(Plot, "UP2", "UP_2"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP3"), 
                        str_replace(Plot, "UP3", "UP_3"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP4"), 
                        str_replace(Plot, "UP4", "UP_4"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP5"), 
                        str_replace(Plot, "UP5", "UP_5"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP6"), 
                        str_replace(Plot, "UP6", "UP_6"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP7"), 
                        str_replace(Plot, "UP7", "UP_7"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP8"), 
                        str_replace(Plot, "UP8", "UP_8"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "UP8"), 
                        str_replace(Plot, "UP8", "UP_8"), 
                        Plot)) %>%
  ...
  mutate(Plot = if_else(str_detect(Plot, "UP8"), 
                        str_replace(Plot, "UP8", "UP_8"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "1aug"), 
                        str_replace(Plot, "1aug", "1-Aug"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "2aug"), 
                        str_replace(Plot, "2aug", "2-Aug"), 
                        Plot)) %>%
  mutate(Plot = if_else(str_detect(Plot, "-U-"), 
                        str_replace(Plot, "-U-", "-UP-"), 
                        Plot)) %>%
  ...
  mutate(Plot = if_else(str_detect(Plot, "-tr-"), 
                        ...
                        Plot)) %>%
  mutate(Plot = gsub(pattern = "-", replacement = "_", x = Plot)) 
