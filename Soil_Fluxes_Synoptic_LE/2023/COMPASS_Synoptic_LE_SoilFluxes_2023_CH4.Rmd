...
alldat <- alldat %>%
  ...
  mutate(
    label = label %>%
      ...
      # <<< FIX >>> ensure ZONE_NUMBER format everywhere (TR31 → TR_31, _TR31 → _TR_31)
...
# ------------------------------------------------------------------
# Extract collar number from the FIXED label
alldat <- alldat %>%
  mutate(
    PlotNum = as.integer(str_extract(label, "(?<=_)\\d+(?=(_|$))"))
  )

# ------------------------------------------------------------------
# Correct zones saved wrongly in the field
alldat <- alldat %>%
  mutate(
    label = case_when(
      Site == "CRC" & Date == as.Date("2023-02-21") & PlotNum %in% c(36, 37) ~
        str_replace(label, "_W_", "_TR_"),

      Site == "CRC" & Date == as.Date("2023-02-21") & PlotNum %in% c(38, 40) ~
        str_replace(label, "_UP_", "_TR_"),

      Site == "CRC" & Date == as.Date("2023-03-24") & PlotNum == 48 ~
        str_replace(label, "_TR_", "_W_"),
